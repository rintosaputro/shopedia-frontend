...
function Cart() {
  ...
  const handleSubtotal = () => {
    if (!statusCounter) {
      const subtotalPrice = listCart.map((item) => {
        let result = 0;
        result += (item.qty * item.data.price);
        return result;
      });

      return subtotalPrice.reduce((total, num) => total + num, 0);
    }
  };
  ...
  useEffect(() => {
    if (cart.isAddCart) {
      dispatch(getProduct);
      const parseCart = JSON.parse(window.localStorage.getItem('cart'));
  ...
  const handleDelete = (index) => {
    const getListcart = listCart.filter((item, indexCart) => indexCart !== index);
                  ...
                  listCart.map((item, index) => (
                    <Row key={index}>
                      ...
                      <Col xs={6} lg={2} className="my-auto mt-4 mt-lg-auto text-center">
                        <span className="fw-bold"><NumberFormat value={String(item.data.price)} prefix="Rp. " mask="." thousandSeparator displayType="text" /></span>
                        {/* <span className="fw-bold">{formatter.format(item.data.price)}</span> */}
                      </Col>
                      <Col xs={6} lg={2} className="my-auto mt-4 mt-lg-auto text-center">
                        <span>
                          <div className="d-flex justify-content-between">
                            <CButton classStyle={cartStyle.button} onClick={() => countDecrement(index)}>-</CButton>
                            <CInput classVariant={cartStyle.formQty} type="number" value={item.qty} />
                            <CButton classStyle={cartStyle.button} onClick={() => countIncrement(index)}>+</CButton>
                          </div>
                        </span>
                      </Col>
                      <Col xs={6} lg={2} className="my-auto mt-4 mt-lg-auto text-center">
                        <span className="text-muted d-inline d-lg-none">Total: </span>
                        <span className="fw-bold"><NumberFormat value={String(item.qty * item.data.price)} prefix="Rp. " mask="." thousandSeparator displayType="text" /></span>
                  ...
                  <Col md>
                    <div className="ms-md-4 ps-5 mt-5 pt-4 bg-color4 ">
                      ...
                      <Row className="mb-5">
                        ...
                        <Col><div className="fw-bold"><NumberFormat value={String(subtotal)} prefix="Rp. " mask="." thousandSeparator displayType="text" /></div></Col>
                      ...
                      <hr className="w-80 me-5" />
                      <Row className="pb-5">
                        <Col xs={4}><div className="fw-bold">Total Price</div></Col>
                        <Col><div className="fw-bold"><NumberFormat value={String(totalPrice())} prefix="Rp. " mask="." thousandSeparator displayType="text" /></div></Col>
                      </Row>
                    </div>
